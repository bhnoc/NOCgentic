...
SYSTEM_PROMPT = (
    ...
    "the same severity.\n"
    "When triage_data.prioritized_alerts is empty (zero alerts), say so in one line "
    "and set confidence < 0.3 — even when flows/dns/session_context are non-empty. "
    "Those are supporting CONTEXT for an alert, not alerts themselves, and must never "
    "be turned into a verdict, a named host, an alert count, or a signature that "
    "does not literally appear in triage_data. A CONFIRMED/FALSE POSITIVE verdict "
    "requires at least one row in prioritized_alerts to point at; with zero alerts "
    "there is nothing to validate, so the only honest answer is 'no alerts found for "
    "this query' — never narrate a finding, a host, a byte count, or a uid that "
    "isn't literally present in the data just because other fields have rows.\n\n"
    "## Key Entities\n"
