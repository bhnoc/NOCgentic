...
SQL_GEN_PROMPT = (
    ...
    "- asset_classification: ip, mac, vendor_mac, device_type, type_name, type_group, brand, "
    "model, os_name, os_ver, confidence (AUTHORITATIVE device inventory; vendor_mac holds the "
    "real OUI-resolved vendor like 'VMware'/'Apple, Inc.'. Keyed by ip AND mac. Use for "
    "'what/whose device is this')\n"
    ...
    "  Generate THREE queries so identity, vendor, and behaviour all surface. Match the MAC "
    "exactly; do not lowercase or reformat it.\n"
    "  (1) asset_classification = the authoritative device inventory. Keyed by BOTH mac and "
    "ip, it carries the real vendor_mac (e.g. 'VMware', 'Apple, Inc.', 'Intel Corporate', "
    "resolved from the OUI in the data), plus device_type, type_name, brand, model, os_name, "
    "confidence. This is where 'what is this device' is answered. Query it FIRST:\n"
    "    SELECT ip, mac, vendor_mac, device_type, type_name, brand, model, os_name, confidence\n"
    "    FROM asset_classification WHERE dt IN ('TODAY','YESTERDAY') AND mac='<mac>'\n"
    "    ORDER BY confidence DESC LIMIT 10\n"
    "  (2) conn = the MAC<->IP link and activity volume (id_orig_mac is reliable; the "
    "id_orig_mac_vendor COLUMN is almost always 'unknown', so IGNORE it and take vendor from "
    "asset_classification / known_devices instead):\n"
    ...
    "  (3) known_devices / dhcp for the self-reported hostname + vendor_mac (may be empty):\n"
    "    SELECT mac, vendor_mac, host_ip, annotations FROM known_devices\n"
    "      WHERE dt IN ('TODAY','YESTERDAY') AND mac='<mac>' LIMIT 10\n"
    "  (4) OUI-vendor fallback for when this exact MAC is not in the inventory (infra hosts "
    "often are not). The vendor is the MAC's OUI (first 3 octets). Resolve it FROM THE DATA "
    "by matching the same OUI prefix on other classified devices, so you never have to guess "
    "and never say 'vendor unknown'. Replace <oui> with the MAC's first 8 chars, e.g. "
    "'00:0c:29':\n"
    "    SELECT vendor_mac, COUNT(*) AS c FROM asset_classification\n"
    "      WHERE dt IN ('TODAY','YESTERDAY') AND substr(mac,1,8)='<oui>'\n"
    "        AND vendor_mac IS NOT NULL AND vendor_mac <> ''\n"
    "      GROUP BY vendor_mac ORDER BY c DESC LIMIT 3\n"
    "  Lead the answer with asset_classification's vendor/device_type/os for the exact MAC "
    "when present, else known_devices.vendor_mac, else the OUI-sibling vendor from (4). Only "
    "if (4) is also empty, name the vendor from well-known OUIs (00:0c:29 / 00:50:56 = "
    "VMware). NEVER report 'vendor unknown'. Also flag a locally-administered/randomized MAC "
