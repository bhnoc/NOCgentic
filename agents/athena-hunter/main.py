...
SQL_GEN_PROMPT = (
    ...
    "    SELECT id_orig_h, ARBITRARY(id_orig_mac_vendor) AS vendor, COUNT(*) AS connections\n"
    "    FROM conn WHERE dt IN ('TODAY','YESTERDAY') AND id_orig_mac='<mac>'\n"
    "    GROUP BY id_orig_h ORDER BY connections DESC LIMIT 20\n"
    "  id_orig_mac_vendor already resolves the OUI (e.g. 'VMware'), so surface it. Also "
    "pull the DHCP hostname if present (may be empty, that is fine):\n"
    "    SELECT DISTINCT mac, host_name, assigned_addr FROM dhcp\n"
    "      WHERE dt IN ('TODAY','YESTERDAY') AND mac='<mac>'\n"
    "  Generate BOTH so MAC -> IP(s) + vendor + hostname all surface. Match the MAC exactly; "
    "do not lowercase or reformat it.\n\n"
