...
SQL_GEN_PROMPT = (
    ...
    "* Fan-out / 'talks to the most other hosts' / relay-detection questions "
    "(ranking hosts by COUNT(DISTINCT <the other side>)): ALWAYS exclude the "
    "non-attributable placeholder addresses '::' (IPv6 unspecified) and '0.0.0.0' "
    "(IPv4 unspecified) from the ranked side. Zeek emits these for unset/unknown "
    "addresses (failed connections, some ICMP/UDP edge cases) — a live bug ranked "
    "'::' as the #1 host by fan-out, ahead of every real host, which is not an "
    "investigable endpoint:\n"
    "    SELECT id_orig_h, COUNT(DISTINCT id_resp_h) AS unique_responders\n"
    "    FROM conn WHERE dt=... AND id_orig_h NOT IN ('::', '0.0.0.0')\n"
    "      AND id_resp_h NOT IN ('::', '0.0.0.0')\n"
    "    GROUP BY id_orig_h ORDER BY unique_responders DESC LIMIT 20\n\n"
    ...
    "5. For 'which hosts have NEVER done X' / exclusion questions, use "
    "`NOT EXISTS (SELECT 1 FROM alerts a WHERE a.orig_h = c.id_orig_h AND ...)`, "
    "never `NOT IN (SELECT orig_h FROM alerts ...)`. alerts.orig_h can be NULL "
    "(some alert sources, e.g. anomaly detections, don't populate a source host), "
    "and `NOT IN` against a subquery containing any NULL silently returns ZERO "
    "rows for every candidate regardless of the true answer. `NOT EXISTS` is "
    "null-safe and does not have this failure mode.\n\n"
...
async def generate_sql(
    ...
    with tracer.start_as_current_span("athena_hunter.generate_sql") as span:
        ...
        user_content = (
            ...
            f"- today's local midnight, as an epoch second, for computing a bare "
            f"clock-time range yourself: {_day_start}\n\n"
            ...
            "- a bare CLOCK-TIME range with no day named (\"between 3am and 4am\", "
            "\"around 2pm\"): the analyst's clock is EVENT-LOCAL, same as \"today\" above, "
            "NOT UTC. Compute the epoch bounds yourself as (today's local midnight epoch, "
            f"given above as {_day_start}) + the number of SECONDS since local midnight "
            "for the start/end clock time (e.g. 3am = 3*3600=10800 seconds after local "
            "midnight, 4am = 4*3600=14400). Do NOT guess or reuse a different query's "
            "epoch bounds for this shape — a live bug generated bounds an hour or more off "
            "for exactly this question shape because no worked example existed for it.\n"
...
SYSTEM_PROMPT = (
    ...
    "- When a result is grouped by zone/segment name and MULTIPLE rows are restricted-"
    "infrastructure segments (Registration Hypervisors, Tool Mgmt, OpenDNS/Umbrella DNS "
    "Virtual Appliances), those distinct real names all display as the single word "
    "'internal' to the analyst after redaction — citing only the largest of several such "
    "rows silently drops the others from the analyst's view (a live bug undercounted a "
    "zone's alert total by ~18% this way). SUM every restricted-infrastructure row into "
    "ONE number before citing a per-zone count; never cite just the largest.\n"
    ...
    "a 'last hour' lens that no longer covers it. When a query SUCCEEDED with a bounded "
    "time window (last hour/today/etc.) and found nothing, name that window in the answer "
    "(e.g. 'No activity from X in the last hour (16:00-17:00)') so a stale-window miss reads "
    "as a scoped result, not a clean bill of health. This rule applies ONLY to a query that "
    "actually ran and returned 0 rows — a FAILED query (see Query Errors / below) never gets "
    "this treatment; follow the failed-query rule instead, and never combine the two in one "
    "sentence.\n\n"
    ...
    "do NOT phrase it as a negative finding ('no X found', 'none observed') and do "
    "NOT name a time window for it — a window applies to a completed search, not a "
    "search that never ran. Say only 'the lookup for <topic> could not be completed' "
    "and set confidence < 0.3. Never combine a failure with a negative-finding sentence, "
